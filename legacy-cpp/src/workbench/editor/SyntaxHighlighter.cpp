...
void SyntaxHighlighter::buildRulesForLanguage() {
    ...
    switch (m_language) {
        ...
        case Language::Json:
            // String keys
            addRule(R"("[^"\\]*(?:\\.[^"\\]*)*"(?=\s*:))", m_keywordFormat);
            // Numbers
            addRule(R"(\b[0-9]+\.?[0-9]*\b)", m_numberFormat);
            // Booleans/null
            addRule(R"(\b(true|false|null)\b)", m_typeFormat);
            break;

        case Language::Markdown:
            // Headers
            addRule(R"(^#{1,6}\s+.*$)", m_keywordFormat);
            // Bold
            addRule(R"(\*\*.*?\*\*)", m_typeFormat);
            // Italic
            addRule(R"(\*.*?\*)", m_functionFormat);
            // Inline code
            addRule(R"(`[^`\n]+`)", m_stringFormat);
            break;
